	...
	<div class="container">
		<div class="row">
			<template v-if="userinfo">
				<p>Olá {{ userinfo.firstname }}, selecione uma consulta para moderar: </p>
			...
			<h3 v-if="selectedConsulta"> Você está moderando: {{ sliceConsultaStringTitle(selectedConsulta) }} </h3>
		</div>
		<div id="isadmin" v-if="isadmin && selectedConsulta">
			<div class="row">
				<div class="section">
					...
					<div class="card" 
						 v-for="(comment, index) in members" 
						 v-if="comment.public == false && comment.trash == false">
						<div class="card-content">
							<p class="date-time">{{ comment.commentdate }} | {{ comment.memid }}</p>
							...
							<div class="card-action">
								<a class="btn-large" 
									@click="changeApproval(comment.memid, comment.public, index)">Aprovar
								</a>
								<a class="btn-flat trash" 
									@click="changeTrashState(comment.memid, comment.trash, index)"><i class="material-icons left"> delete</i>Lixeira
								</a>
							</div>
						</div>
					</div> 
			...
			<div class="row">
				<div id="approved" class="section">
					...
					<div class="card" 
						...
						<div class="card-content">
							<p class="date-time">{{ comment.commentdate }} | id:{{ comment.memid }}</p>
							<p class="card-title">Seção: {{ comment.postid }}</p>
								<blockquote><p><span>Contexto: </span>{{ comment.commentcontext }}{{ comment.memid }}</p></blockquote>
							<p><span>Autor:</span> {{ comment.name }} | <a v-bind:href=" `mailto:${comment.email}` ">{{ comment.email }}</a></p>
							<div>
								<p><span>Comentário: </span>{{ comment.content }}</p>
							</div>
							<div class="card-action">
								<a class="btn-flat trash" 
								   @click="changeApproval(comment.memid, comment.public, index)"><i class="material-icons left"> undo</i>Moderar novamente
								</a>
							</div>
						</div>
					</div> 
				...
				<div id="trash" class="section">
					...
					<div class="card" 
						...
						<div class="card-content">
							...
							<div class="card-action">
								<a class="btn-flat trash" 
								   @click="changeTrashState(comment.memid, comment.trash, index)"><i class="material-icons left"> undo</i>Moderar novamente
								</a>
							</div>
						</div>
					</div> 
...
export default {
	...
	methods: {
		...
		changeApproval(id, approval, index) {
			...
			requestConsultas.post('member_update/', memForm)
				.then(function(response){
					if(response.data.error){
						...
						app.message = response.data.message
						app.members[index].public = approval
					}
				}
			);
		},

		changeTrashState(id, approval, index) {
			...
			this.members[index].trash = approval
		},
